...
public class Game {
    ...
    public boolean canMove(int x, int y, int destX, int destY, Side s){
       ...
       - piece must move "through" a piece to get from (x,y) to (destX,destY) (use isVisible())
        ...
        return false;
        Piece piece1=b.get(x, y);
        if(piece1==null) return false;
        if(x==destX && y==destY) return false;
        if(piece1.getSide()!=s) return false;
        if(!piece1.canMove(destX, destY)) return false;
        Piece piece2=b.get(destX, destY);
        if(piece2!=null && piece2.getSide()==piece1.getSide()) return false;
        if(!(piece1 instanceof Knight))
        {
            if(isVisible(x, y, destX, destY)==false) return false;
        }
    ...
    public boolean move(int x, int y, int destX, int destY){
        ...
        if(piece2!=null)
        {
            if(piece2==b.getKing(Side.negate(currentTurn)))
            {
                appendWinToHistory(currentTurn);
                piece1.move(destX,destY);
                return true;
            }
        }
        piece1.move(destX,destY);
        if(isInCheck(Side.negate(currentTurn))) appendCheckToHistory(Side.negate(currentTurn));
    ...
    public boolean isInCheck(Side side) {
        // TODO write this method
        List<Piece> opponents=b.getPieces(Side.negate(side));
        int kingX=b.getKing(side).x;
        int kingY=b.getKing(side).y;
        for(Piece  opponent: opponents)
        {
            if(canMove(opponent.x, opponent.y, kingX, kingY, Side.negate(side))) return true;
        }
        return false;
    }
